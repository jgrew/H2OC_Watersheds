...
import { d as getSlotted, g as getElementDir } from './dom-9b1c8e39.js';
import { C as CalciteHeading, a as ConstrainHeadingLevel } from './CalciteHeading-8935aa0f.js';
...
const HEADING_LEVEL$1 = (HEADING_LEVEL + 1);
...
const CalciteTip = class {
  ...
  renderHeader() {
    ...
    return heading ? (h("header", { class: CSS$1.header }, h(CalciteHeading, { class: CSS$1.heading, level: headingLevel }, heading))) : null;
  }
  renderDismissButton() {
    const { nonDismissible, hideTip, intlClose } = this;
    const text = intlClose || TEXT$1.close;
    return !nonDismissible ? (h("calcite-action", { class: CSS$1.close, icon: ICONS$1.close, onClick: hideTip, scale: "l", text: text })) : null;
  }
  renderImageFrame() {
    const { el } = this;
    return getSlotted(el, SLOTS.thumbnail) ? (h("div", { class: CSS$1.imageFrame }, h("slot", { name: SLOTS.thumbnail }))) : null;
  }
  renderInfoNode() {
  ...
  render() {
    return (h(Host, null, h("article", { class: CSS$1.container, hidden: this.dismissed }, this.renderHeader(), this.renderContent()), this.renderDismissButton()));
  }
