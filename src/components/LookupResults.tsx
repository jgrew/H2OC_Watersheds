...
class LookupResults extends (Widget) {
	...
	_drainsTo=[
		...
		{to:"Anaheim Bay / Huntingon Harbor",watershed:"Anaheim Bay"},
    ...
    render() {
		const loader =
			this.state === 'loading' ? (
                <div style="display: inline-flex;align-items: center;justify-content: center;width: 100%;">
                    <calcite-loader scale="m" inline="" active=""></calcite-loader>
		...
		const error = this.state === 'error' || 'warn' ? (
			this.createErrorDiv()
		) : null;

		const results = this.state === 'ready' ? (
    ...
    queryFeatures(location: esri.Graphic, searchPoint: esri.ScreenPoint) {
        ...
        this.rsbLayer.queryFeatures(this._rsbQuery).then(result => {
            
			if (result.features.length > 0) {
				this._empty = false;
				this._catchidn = result.features[0].attributes['CatchIDN'];
				this._watershed = result.features[0].attributes['Watershed'];
				...
				// const dsQuery: esri.Query = this.rsbDSLayer.createQuery();
	...
	createResultDiv() {
		...
		let drainsTo = this._drainsTo.find(el => el.watershed === this._watershed);

		return (
			this._empty ? null : <div class="panel animate-fade-in">
				<div class="phone-column-6 tablet-column-12 column-10 padding-left-0 padding-right-0">
					<h3>Search Results</h3>
					<p>The location you selected is in drainage basin <strong><button class="btn-link" bind={this} onclick={this._zoomToFeature}>#{this._catchidn}</button></strong>. It is <strong>{Math.round(this._areaacre)}</strong> acres in size, is part of the <strong>{this._watershed}</strong> watershed, and drains to <strong>{drainsTo!["to"]}</strong>.</p>
					<button class="btn btn-green" bind={this} onclick={this._zoomToDSFeature}>Zoom to downstream</button>
				</div>
			</div>
		)
	}
	...
	createErrorDiv() {
		return (
			this.state === 'warn' ? <div class="alert .alert-yellow is-active"> No results found
			<button class="alert-close">
				<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32" class="svg-icon">
				<path d="M18.404 16l9.9 9.9-2.404 2.404-9.9-9.9-9.9 9.9L3.696 25.9l9.9-9.9-9.9-9.898L6.1 3.698l9.9 9.899 9.9-9.9 2.404 2.406-9.9 9.898z"/>
				</svg>
			</button></div> : <div class="alert .alert-red is-active">
	...
	createPost() {
		return (
			<button class="btn">Switch to map</button>
		)
	}

	_createLink(node: HTMLElement) {
		node.addEventListener('click', () => {
			this.rsbDSLayer.when(function() {
	...
	_zoomToDSFeature = () => {
		this.view.whenLayerView(this.rsbDSLayer as esri.FeatureLayer).then(layerView => {
	...
	_zoomToFeature = () => {
		this.view.whenLayerView(this.rsbLayer as esri.FeatureLayer).then(layerView => {
			layerView.queryExtent().then(response => {
				this.view.goTo(response.extent);
			})
		})
